...
// ── BUG6: cards bounce between In Review and In Progress ─────────────

/** Assistant text record carrying the API stop_reason, as Claude Code writes it. */
export function assistantTurn(text: string, minutesAgo: number, stopReason: 'end_turn' | 'tool_use'): string {
  return line({
    type: 'assistant',
    uuid: crypto.randomUUID(),
    timestamp: ts(minutesAgo),
...
/** tool_result record with the entry-level toolUseResult Claude Code attaches. */
export function toolResult(text: string, minutesAgo: number, toolUseResult: Record<string, unknown>): string {
  return line({
    type: 'user',
    uuid: crypto.randomUUID(),
    timestamp: ts(minutesAgo),
...
/**
 * A background task's task-notification. Claude Code writes it as a user
 * record when it wakes an idle session, and as queue-operation + attachment
 * records when the session is mid-turn. Monitor events carry no <status>.
 */
export function taskNotification(taskId: string, status: string | undefined, minutesAgo: number, as: 'user' | 'queue' | 'attachment' = 'user'): string {
  const body = `<task-notification>\n<task-id>${taskId}</task-id>\n`
    + (status ? `<status>${status}</status>\n<summary>Task finished</summary>\n` : '<summary>Monitor event</summary>\n<event>tick</event>\n')
    + '</task-notification>';
  const base = { uuid: crypto.randomUUID(), timestamp: ts(minutesAgo), sessionId: 'test-session' };
  if (as === 'queue') return line({ ...base, type: 'queue-operation', operation: 'enqueue', content: body });
  if (as === 'attachment') return line({ ...base, type: 'attachment', attachment: { type: 'queued_command', prompt: body } });
  return line({
    ...base,
    type: 'user',
    parentUuid: null,
...
/** A local slash command (e.g. /model): three user records, no assistant reply. */
export function localCommand(name: string, output: string, minutesAgo: number): string {
  const user = (content: string, extra: Record<string, unknown> = {}) => line({
    type: 'user',
    uuid: crypto.randomUUID(),
    timestamp: ts(minutesAgo),
    sessionId: 'test-session',
...
/** Turn ended while a background Bash command is still running. */
export const backgroundBashPendingConversation = [
  userMessage('Run the full benchmark', 20),
  assistantMessage('Starting it in the background.', 19, [{ name: 'Bash' }]),
  toolResult('Command running in background with ID: b1abc. Output is being written to: /tmp/b1abc.output', 19, { backgroundTaskId: 'b1abc' }),
  assistantTurn("It's running; I'll report the numbers when it finishes.", 18, 'end_turn'),
].join('\n');

/** Turn ended while a background agent is still running. */
export const backgroundAgentPendingConversation = [
  userMessage('Survey the finish schedules', 20),
  assistantMessage('', 19, [{ name: 'Agent' }]),
  toolResult('Async agent launched successfully.', 19, { isAsync: true, status: 'async_launched', agentId: 'a9f00d' }),
  assistantTurn('The survey agent is running. Completed setup successfully.', 18, 'end_turn'),
].join('\n');
